...
> 历史完成项（多进程部署 / 量化评估 / 截图等）见 `docs/改造计划.md` M6、M8；下表 I-01「预览像素差」已于 M10 解决。
...
### 13.4 Web 形态压力摸底（M17，scripts/stress_web.py）

测法：进程内起 uvicorn + mock LLM 每节点延时 8s 拉长任务窗口；客户端与服务端同进程（GIL 竞争使结果偏保守，作下界参考）。数据全部落在 `tests/.tmp/stress/`，不碰运行时目录。

**SSE 长连接并发**（单任务阶梯加连接，每波保持 7s）：

| 并发数 | 连上 | 首包 p50 | 首包 p95 | 错误 | 进程内存 |
| --- | --- | --- | --- | --- | --- |
| 25 | 25 | 0.07s | 0.08s | 0 | 139 MB |
| 50 | 50 | 0.11s | 0.25s | 0 | 196 MB |
| 100 | 100 | 0.27s | 0.34s | 0 | 210 MB |
| 150 | 150 | 0.35s | 0.47s | 0 | 218 MB |

结论：单进程 150 路并发 SSE 全部连上且首包 p95 < 0.5s，内存线性增长约 0.6 MB/连接；叠加 10 分钟兜底断开机制，观察连接承载力远超单用户场景所需。

**SQLite WAL 并发写**（模拟 0.8s 进度节流写的极端化：持续 commit）：

| 工作循环 | 时长 | 成功写 | 吞吐 | 错误 |
| --- | --- | --- | --- | --- |
| 20 | 8.4s | 3048 | 362 ops/s | 1（database is locked） |
| 60 | 8.4s | 3429 | 407 ops/s | 0 |
| 120 | 8.6s | 3335 | 389 ops/s | 1（database is locked） |

结论：吞吐稳定在 ~350-400 ops/s（瓶颈在连接池 5+10 的排队而非 SQLite 本身）；`busy_timeout=5000` 下偶发 locked 约 0.03%（3000+ 次中 1 次），且进度写属可覆盖的非关键数据；实际业务流量（每任务约 1 写/0.8s）距该上限有 3 个数量级余量。

### 13.5 展望（后续优化方向）

- **论文级**：岗位能力知识库混合检索（结构化行业考察点卡片 + 证据池双路）；接入免费 embedding（如 bge-m3）跑通向量链路对比关键词召回；评估规模化（30+ 用例 + LLM-as-judge 置信区间）
- **工程级（截至 M17）**：~~checkpointer 换 SqliteSaver~~（M16 完成）、~~追问回答本地草稿暂存~~（M16 完成）、~~web 形态 SSE 压测~~（M17 完成，见 13.4）；scorer 并行化经核实为伪优化（单次 LLM 调用评六维）不做；剩余：exe 自动更新检查
- **明确不做**：海量文档库 RAG（知识在用户输入内闭合，见 6.8 设计定位）、用户注册体系（匿名会话已够）、微服务拆分

---

*本文档依据项目当前代码状态编写（v3.1，对应 M17 完成节点），随版本迭代同步更新。*
